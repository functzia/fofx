...
const getManifests = wrapByDevNull(async (specNames, install) => {
...
async function getPluginsByType(pluginsFile, log, install = true) {
  ...
  const manifests = await getManifests(specNames, install);
